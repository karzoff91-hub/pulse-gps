...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      const { playerName, testName, value, unit, notes, testDate } = req.body || {};
      if (!playerName || !testName || value === undefined) {
        return res.status(400).json({ error: 'playerName, testName e value sono obbligatori.' });
      }
      const { rows } = await client.query(
        `INSERT INTO rehab_tests (player_name, test_name, value, unit, notes, test_date)
         VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`,
        [playerName, testName, String(value), unit || null, notes || null, testDate || null]
      );
