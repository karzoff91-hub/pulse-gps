...
// PATCH  -> modifica un test esistente { id, testName, value, unit, notes }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'PATCH') {
      const { id, testName, value, unit, notes } = req.body || {};
      ...
      const { rows } = await client.query(
        ...
        [id, testName || null, value !== undefined ? String(value) : null, unit || null, notes || null]
      );
      return res.status(200).json({ Result: 'OK', Test: rows[0] });
    }
