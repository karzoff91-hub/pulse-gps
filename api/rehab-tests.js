...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      if (!playerName) {
        // Solo consentito per i test performance e le voci di benessere:
        // tutti i giocatori insieme (usato dalle viste Team Test / Carico
        // Interno). Per "rehab" (dati medici) resta obbligatorio un
        // giocatore specifico, per non esporre tutto lo storico insieme.
        if (category !== 'performance' && category !== 'benessere') {
          return res.status(400).json({ error: 'Parametro playerName mancante.' });
        }
        const { rows } = await client.query(
          `SELECT * FROM rehab_tests WHERE category = $1 ORDER BY test_date DESC NULLS LAST, created_at DESC`,
          [category]
        );
