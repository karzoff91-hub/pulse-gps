  ...
  try {
    if (req.method === 'POST') {
      ...
      const { rows } = await client.query(
        ...
        [sheetId, exerciseName, youtubeUrl || null, kgParsed.value, reps || null, recovery || null, notes || null]
      );
    ...
    if (req.method === 'PATCH') {
      ...
      const { rows } = await client.query(
        ...
        [id, exerciseName || null, youtubeUrl || null, kgParsedStaff.value, reps || null, recovery || null, notes || null]
      );
      return res.status(200).json({ Result: 'OK', Exercise: rows[0] });
    }
