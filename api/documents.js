...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST' && req.body && req.body.isProfile) {
      const { playerName, heightCm, weightKg, age } = req.body;
    ...
    if (req.method === 'POST') {
      const { playerName, docType, title, fileBase64, fileName, mimeType, injuryId } = req.body || {};
      ...
      const { rows } = await client.query(
        `INSERT INTO player_documents (player_name, doc_type, title, blob_url, mime_type, injury_id) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`,
        [playerName, docType, title || fileName, blob.url, finalMime, injuryId || null]
      );
