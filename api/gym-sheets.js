...
//        -> ?action=token_regenerate&playerName=... invalida il precedente e ne crea uno nuovo
//        -> ?action=staff_token/staff_token_regenerate&role=... idem per il link fisio squadra
//        -> ?action=physio_token/physio_token_regenerate&playerName=... idem per il fisio privato
...
async function ensureTables(client){
  ...
  await client.query(`ALTER TABLE gym_sheets ADD COLUMN IF NOT EXISTS source TEXT DEFAULT 'staff';`);
  await client.query(`
  ...
  `);
  await client.query(`ALTER TABLE gym_exercises ADD COLUMN IF NOT EXISTS key_lift_tag TEXT;`);
  await client.query(`ALTER TABLE gym_exercises ADD COLUMN IF NOT EXISTS sets INTEGER;`);
  await client.query(`ALTER TABLE gym_exercises ADD COLUMN IF NOT EXISTS reps_count INTEGER;`);
  await client.query(`
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      if (action === 'token_regenerate') {
        if (!playerName) return res.status(400).json({ error: 'Parametro playerName mancante.' });
        const newToken = crypto.randomBytes(16).toString('hex');
        await client.query(
          `INSERT INTO player_tokens (player_name, token) VALUES ($1, $2)
           ON CONFLICT (player_name) DO UPDATE SET token = $2`,
          [playerName, newToken]
        );
        return res.status(200).json({ Result: 'OK', token: newToken });
      }
      ...
      if (action === 'staff_token_regenerate') {
        const { role } = req.query;
        if (!role) return res.status(400).json({ error: 'Parametro role mancante.' });
        const newToken = crypto.randomBytes(16).toString('hex');
        await client.query(
          `INSERT INTO staff_tokens (role, token) VALUES ($1, $2)
           ON CONFLICT (role) DO UPDATE SET token = $2`,
          [role, newToken]
        );
        return res.status(200).json({ Result: 'OK', token: newToken });
      }
      ...
      if (action === 'physio_token_regenerate') {
        if (!playerName) return res.status(400).json({ error: 'Parametro playerName mancante.' });
        const newToken = crypto.randomBytes(16).toString('hex');
        await client.query(
          `INSERT INTO player_physio_tokens (player_name, token) VALUES ($1, $2)
           ON CONFLICT (player_name) DO UPDATE SET token = $2`,
          [playerName, newToken]
        );
        return res.status(200).json({ Result: 'OK', token: newToken });
    ...
    if (req.method === 'POST') {
      const { playerName, title, startDate, source } = req.body || {};
      if (!playerName) return res.status(400).json({ error: 'playerName obbligatorio.' });
      const allowedSources = ['staff', 'fisio_squadra', 'fisio_privato'];
      const finalSource = allowedSources.includes(source) ? source : 'staff';
      await client.query(
      ...
      const defaultTitle = finalSource === 'staff' ? ('Scheda ' + n) : ('Scheda riabilitativa ' + n);
      const { rows } = await client.query(
        'INSERT INTO gym_sheets (player_name, title, is_current, start_date, source) VALUES ($1, $2, true, COALESCE($3, CURRENT_DATE), $4) RETURNING *',
        [playerName, title || defaultTitle, startDate || null, finalSource]
      );
    ...
    if (req.method === 'PATCH') {
      ...
      // Aggiorno SOLO i campi effettivamente presenti nella richiesta — così
      // rinominare una scheda non azzera più per sbaglio le date del periodo,
      // e viceversa.
      const sets = [];
      const vals = [id];
      if ('title' in body) { vals.push(body.title || null); sets.push(`title = $${vals.length}`); }
      if ('startDate' in body) { vals.push(body.startDate || null); sets.push(`start_date = $${vals.length}`); }
      if ('endDate' in body) { vals.push(body.endDate || null); sets.push(`end_date = $${vals.length}`); }
      if (sets.length === 0) return res.status(400).json({ error: 'Nessun campo da aggiornare.' });
      const { rows } = await client.query(
        `UPDATE gym_sheets SET ${sets.join(', ')} WHERE id = $1 RETURNING *`,
        vals
      );
      return res.status(200).json({ Result: 'OK', Sheet: rows[0] });
    }
