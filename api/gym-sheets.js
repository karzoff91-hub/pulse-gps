...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      if (currentRows.length === 0) {
        const { rows: created } = await client.query(
          'INSERT INTO gym_sheets (player_name, title, is_current) VALUES ($1, $2, true) RETURNING *',
          [playerName, 'Scheda 1']
        );
        current = created[0];
      } else {
    ...
    if (req.method === 'POST') {
      const { playerName, title } = req.body || {};
      if (!playerName) return res.status(400).json({ error: 'playerName obbligatorio.' });
      await client.query('UPDATE gym_sheets SET is_current = false WHERE player_name = $1 AND is_current = true', [playerName]);
